...
| ファイル／配置場所                                                     | 役割                                                                                                                                                                                                                                                                          | 適用範囲・読み込まれ方                                                                                                                                                                                | 主な記載内容                                                                                                                      | 使いどころ                                                                                                                 |
| ---------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `AGENTS.md`<br>または<br>`.github/copilot-instructions.md`             | **リポジトリ全体で守る共通ルールを定義する。** AI向けのREADMEに相当し、プロジェクトの前提・方針・禁止事項・開発規約などをまとめる。`AGENTS.md` は複数のAIコーディングツールで共有しやすいクロスプラットフォーム形式、`.github/copilot-instructions.md` はGitHub Copilot専用。 | プロジェクトルートまたは配下のディレクトリを対象とし、原則として作業中に常時参照される。必要に応じて `@filepath` で詳細な指示ファイルを読み込める。                                                   | プロジェクト概要、使用技術、ディレクトリ構成、ビルド・テスト方法、命名規則、セキュリティ方針、変更してはいけない箇所など。        | すべての作業に共通する「このリポジトリでは必ず守ること」を定めたい場合。                                                   |
| `.agents/skills/**/SKILL.md`<br>または<br>`.github/skills/**/SKILL.md` | **特定タスクの実行方法やチェックリストを、再利用可能なスキルとして定義する。** エージェントの人格ではなく、「どういう作業を、どの手順・基準で行うか」をパッケージ化する。                                                                                                     | ユーザーの依頼と `description` が一致したときに自動選択されるほか、スキル名を指定して明示的に呼び出せる。スキルごとにフォルダーを分け、必要に応じて `scripts/`、`examples/`、参考資料などを同梱する。 | YAML frontmatter の `name`、`description`、`license` と、作業手順、判断基準、チェック項目、利用するスクリプトや成果物の形式など。 | テスト生成、セキュリティ監査、コード品質チェック、資料作成など、複数のエージェントや案件から繰り返し使う定型作業。         |
| `.github/agents/*.agent.md`                                            | **名前付きの専門エージェントを定義する。** 特定分野の専門担当として、役割、観点、使用可能なツール、振る舞いをまとめる。                                                                                                                                                       | 必要な場面で `/agent` または `copilot --agent <name>` により選択・切り替える。エージェントごとに独自のコンテキストを持ち、選択はセッション単位。                                                      | YAML frontmatter の `description`（必須）、`name`、`tools`、`target` などと、専門領域、優先事項、レビュー観点、回答方針。         | Pythonレビュー担当、セキュリティ担当、テスト担当など、作業ごとに「専門家の役割」へ切り替えたい場合。                       |
| `.github/instructions/*.instructions.md`                               | **技術・ファイル種別・領域ごとの細かなルールを分割して定義する。** リポジトリ全体の共通ルールを肥大化させず、必要な指示だけを適用する。                                                                                                                                       | YAML frontmatter の `applyTo` に指定したglobパターンに一致するファイルを扱うときに適用される。`applyTo` がなければ原則すべての会話に適用される。                                                      | Python、TypeScript、テスト、SQL、Dockerなどのコーディング規約、ファイル別の実装ルール、レビュー基準。                             | `**/*.py` にだけPEP 8を適用する、`tests/**` にだけテスト規約を適用するなど、対象を限定してコンテキスト消費を抑えたい場合。 |

> 💡 SKILL.md は、ローカルフォルダに置いたものをコマンドで `copilot skill add <SKILL.md へのパス>` でも追加可能。
